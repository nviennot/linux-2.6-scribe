...
({									\
	...
	__chk_user_ptr(ptr);						\
	might_fault();							\
	scribe_pre_uaccess(&__val_gu, ptr, sizeof(*(ptr)),		\
			   SCRIBE_DATA_INPUT);				\
	switch (sizeof(*(ptr))) {					\
	case 1:								\
...
({								\
	...
	__chk_user_ptr(ptr);					\
	might_fault();						\
	scribe_pre_uaccess(&__pu_val, ptr, sizeof(*(ptr)), 0);	\
	__pu_val = x;						\
	switch (sizeof(*(ptr))) {				\
	case 1:							\
...
do {									\
	__typeof__(*(ptr)) __pu_val;					\
	__chk_user_ptr(ptr);						\
	scribe_pre_uaccess(&__pu_val, ptr, size, 0);			\
	__pu_val = x;							\
	switch (size) {							\
	case 1:								\
...
do {									\
	__chk_user_ptr(ptr);						\
	scribe_pre_uaccess(&(x), ptr, size, SCRIBE_DATA_INPUT);		\
	switch (size) {							\
	case 1:								\
